...
package com.datamountaineer.streamreactor.connect.influx.avro
...
object AvroRecordFieldExtractorMapFn {
  /**
    ...
    */
  def apply(schema: Schema, fields: Seq[String]): Map[String, (Any) => Any] = {
    fields.map { fn =>
  ...
  private def getFunc(schema: Schema): (Any) => Any = {
    val `type` = schema.getType.getName
    `type`.toUpperCase() match {
      case "BOOLEAN" | "BYTES" | "DOUBLE" | "FLOAT" | "INT" | "LONG" | "STRING" => (v: Any) => if (v == null) null else v
      case "UNION" =>
        schema.getTypes.asScala.collectFirst {
          case s if s.getType != Schema.Type.NULL => getFunc(s)
