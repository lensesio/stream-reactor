...
//
//class RedisDbWriterTestRetry extends WordSpec with Matchers with BeforeAndAfterAll with MockitoSugar {
//
...
//      when(config.getString(EXPORT_MAPPINGS)).thenReturn(s"{topic:$tableName;*}")
//      when(config.getString(ROW_KEYS)).thenReturn("{topic:firstName}")
//      when(config.getString(ERROR_POLICY)).thenReturn("RETRY")
