...
class JsonSqlTest extends AnyWordSpec with Matchers {
  ...
  "JsonSql" should {
    ...
    "handle 'SELECT name, address.street.name as streetName, address.street2.name as streetName2' from record" in {
      val person = Person("Rick", Address(Street("Rock St"), None, "MtV", "CA", "94041", "USA"))
      val actual = JacksonJson.asJson(person).sql(
        "SELECT name, address.street.name as streetName, address.street2.name as streetName2",
      )
    ...
    "handle 'SELECT address.state as S, address.city as C,name, address.street.name' from record" in {
      val person = Person("Rick", Address(Street("Rock St"), None, "MtV", "CA", "94041", "USA"))
      val actual =
        JacksonJson.asJson(person).sql("SELECT address.state as S, address.city as C,name, address.street.name")
    ...
    "not throw an exception if the field doesn't exist in the schema" in {
      val person =
        Person("Rick", Address(Street("Rock St"), Some(Street("Sunset Boulevard")), "MtV", "CA", "94041", "USA"))
    ...
    "handle 'SELECT street as S, city, state, zip as Z, country as C  FROM simpleAddress' from record" in {
      val address = SimpleAddress("Rock St", "MtV", "CA", "94041", "USA")
      val actual =
        JacksonJson.asJson(address).sql("SELECT street as S, city, state, zip as Z, country as C  FROM simpleAddress")
    ...
    "handle 'SELECT address' from record" in {
      ...
      val expected = JacksonJson.asJson(
        "{\"address\":{\"street\":{\"name\":\"Rock St\"},\"street2\":{\"name\":\"Sunset Boulevard\"},\"city\":\"MtV\",\"state\":\"CA\",\"zip\":\"94041\",\"country\":\"USA\"}}",
      )
      compare(actual, expected)
    }
  }
