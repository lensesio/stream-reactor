...
object OffsetMergeUtils {
  ...
  def createCommitContextForEvaluation[B <: BatchRecord](
    batch:                Seq[B],
    currentCommitContext: HttpCommitContext,
  ): HttpCommitContext = {
    val count          = batch.size.toLong
    val fileSize       = batch.map(_.length).sum.toLong
  ...
  def maxRecord[B <: BatchRecord](batch: Seq[B]): B =
    batch.maxBy(_.topicPartitionOffset.offset)

  private def maxOffsets[B <: BatchRecord](batch: Seq[B]): Map[TopicPartition, Offset] =
    batch
