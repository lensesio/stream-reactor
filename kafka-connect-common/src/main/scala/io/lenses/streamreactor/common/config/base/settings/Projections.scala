...
case class Projections(
  targets:          Map[String, String], // source -> target
...
object Projections extends StrictLogging {
  ...
  def getPrimaryKeyCols(kcql: Set[Kcql]): Map[String, Set[String]] =
    kcql.toList
      .map(k => (k.getSource, ListSet(k.getPrimaryKeys.asScala.map(p => p.getName).reverse.toSeq: _*)))
      .toMap
      ...
      .map(
        {
          case (source, clazz) =>
            ...
            val converter = Try(Class.forName(clazz).getDeclaredConstructor().newInstance()) match {
              ...
              case Failure(_) =>
                throw new ConfigException(s"Invalid KCQL is invalid for [$source]. [$clazz] should have an empty ctor!")
            }
