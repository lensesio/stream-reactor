...
object AvroSerializer {
  def write[T <: Product](t: T)(implicit os: OutputStream, formatter: RecordFormat[T], schemaFor: SchemaFor[T]): Unit = write(apply(t), schemaFor.schema)
  ...
  def getBytes[T <: Product](t: T)(implicit recordFormat: RecordFormat[T], schema: Schema): Array[Byte] = getBytes(recordFormat.to(t), schema)
  ...
  def read[T <: Product](is: InputStream)(implicit schemaFor: SchemaFor[T], recordFormat: RecordFormat[T]): T = recordFormat.from(read(is, schemaFor.schema))
