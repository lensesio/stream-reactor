...
case class FieldAdapter(field: Field) extends AbstractField {
  private val fieldSchema = SchemaAdapter(field.schema())
  override def name(): String = field.name()

  override def schema(): AbstractSchema = fieldSchema
}
