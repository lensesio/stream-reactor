...
//  ScandiumLogger.setLevel(Level.INFO)
  ...
  "should create a DTSConnection with Public/Private PEM keys" in {
    ...
    val connection = DTLSConnectionFn(settings.head)
    connection.isLeft shouldBe true
    val dtls = connection.left
    val secret = dtls.get.getPskStore.getKey("andrew")
    val p = new String(secret)
    p shouldBe "kebab"
  }
}
