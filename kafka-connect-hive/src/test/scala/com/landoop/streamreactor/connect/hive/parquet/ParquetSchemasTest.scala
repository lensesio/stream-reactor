...
class ParquetSchemasTest extends AnyFlatSpec with Matchers {
  ...
  it should "support 32 bit ints" in {
    ...
    ParquetSchemas.toKafka(Types.required(PrimitiveTypeName.INT32).as(LogicalTypeAnnotation.intType(32, false)).named("foo")) shouldBe Schema.INT32_SCHEMA
  }
  ...
  it should "support 64 bit longs" in {
    ...
    ParquetSchemas.toKafka(Types.required(PrimitiveTypeName.INT64).as(LogicalTypeAnnotation.intType(64, false)).named("foo")) shouldBe Schema.INT64_SCHEMA
  }
