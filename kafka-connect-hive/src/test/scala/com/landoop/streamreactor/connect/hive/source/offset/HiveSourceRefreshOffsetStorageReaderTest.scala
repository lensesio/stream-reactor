...
class HiveSourceRefreshOffsetStorageReaderTest extends AnyFlatSpec with Matchers with MockitoSugar {
  ...
  private val refreshReaderPartition = SourcePartition(DatabaseName("databaseName"), TableName("tableName"), Topic("topic"), new Path("path1"))
  private val refreshReaderOffset = SourceOffset(100)

  private val initReaderPartition = SourcePartition(DatabaseName("databaseName"), TableName("tableName"), Topic("topic"), new Path("path2"))
  private val initReaderOffset = SourceOffset(200)

  private val unknownPartition = SourcePartition(DatabaseName("databaseName"), TableName("tableName"), Topic("topic"), new Path("path3"))
  ...
  private val target = new HiveSourceRefreshOffsetStorageReader(Map(refreshReaderPartition -> refreshReaderOffset), delegate)
