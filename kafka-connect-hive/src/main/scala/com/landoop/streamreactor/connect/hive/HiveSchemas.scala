  ...
  def toHiveType(schema: Schema): String =
    schema.`type`() match {
      ...
      case Schema.Type.STRUCT =>
        val fields_string = schema.fields.asScala.map { field =>
          s"${field.name}:${toHiveType(field.schema())}"
        }.mkString(",")
        s"struct<$fields_string>"
      case _ => throw UnsupportedHiveTypeConversionException(s"Unknown data type ${schema.`type`}")
    }

  def toKafka(table: Table): Schema =
    toKafka(table.getSd.getCols.asScala.toSeq, table.getPartitionKeys.asScala.toSeq, table.getTableName)
