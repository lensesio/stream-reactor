...
object ParquetSchemas {
  ...
  def toKafka(t: PrimitiveType): Schema = {
    ...
    def int32(original: LogicalTypeAnnotation): SchemaBuilder = original match {
      case lta : LogicalTypeAnnotation.IntLogicalTypeAnnotation if lta.getBitWidth == 32 => SchemaBuilder.`type`(Schema.Type.INT32)
      case lta : LogicalTypeAnnotation.IntLogicalTypeAnnotation if lta.getBitWidth == 16 => SchemaBuilder.`type`(Schema.Type.INT16)
      case lta : LogicalTypeAnnotation.IntLogicalTypeAnnotation if lta.getBitWidth == 8 => SchemaBuilder.`type`(Schema.Type.INT8)
    ...
    def int64(original: LogicalTypeAnnotation): SchemaBuilder = original match {
      case lta : LogicalTypeAnnotation.IntLogicalTypeAnnotation if lta.getBitWidth == 64 => SchemaBuilder.`type`(Schema.Type.INT64)
