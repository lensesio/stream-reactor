...
/**
  ...
  */
case class CommitContext(tpo: TopicPartitionOffset, path: Path, count: Long, fileSize: Long, createdTimestamp: Long)
...
/**
  ...
  */
case class DefaultCommitPolicy(fileSize: Option[Long], interval: Option[FiniteDuration], fileCount: Option[Long])
    extends CommitPolicy {
  require(fileSize.isDefined || interval.isDefined || fileCount.isDefined)
  override def shouldFlush(context: CommitContext)(implicit fs: FileSystem): Boolean = {
