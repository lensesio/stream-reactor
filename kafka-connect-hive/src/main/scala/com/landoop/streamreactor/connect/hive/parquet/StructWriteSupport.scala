...
class StructWriteSupport(schema: Schema) extends WriteSupport[Struct] {
  ...
  override def init(conf: Configuration): WriteSupport.WriteContext =
    new WriteSupport.WriteContext(parquetSchema, new java.util.HashMap[String, String])
  ...
  def valueWriter(schema: Schema): ValueWriter =
    ...
    schema.`type`() match {
      ...
      case Schema.Type.INT8 | Schema.Type.INT16 | Schema.Type.INT32 => value =>
          consumer.addInteger(value.toString.toInt)
