              ...
              map.get(seg) match {
                ...
                case other =>
                  throw new ConfigException(
                    s"The key $longKey is not supported for type ${Option(other).map(_.getClass.getName).getOrElse("NULL")}",
                  )
              }
            }
            case _ =>
              getValue(remainingSegments.tail, map.get(remainingSegments.head).asInstanceOf[java.util.Map[String, Any]])
          }
