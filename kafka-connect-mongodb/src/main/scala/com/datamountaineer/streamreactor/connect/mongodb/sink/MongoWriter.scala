...
class MongoWriter(settings: MongoSettings, mongoClient: MongoClient)
  ...
  private val collectionMap = settings.kcql
    .map(c =>
      c.getSource -> Option(database.getCollection(c.getTarget)).getOrElse {
  ...
  private def insert(records: Seq[SinkRecord]) =
    try {
      records.groupBy(_.topic()).foreach {
        case (topic, groupedRecords) =>
          val collection = collectionMap(topic)
          val config =
            configMap.getOrElse(topic, throw new ConnectException(s"$topic is not handled by the configuration."))
          val batchSize =
            if (config.getBatchSize == 0) MongoConfigConstants.BATCH_SIZE_CONFIG_DEFAULT else config.getBatchSize
  ...
  def apply(connectorConfig: MongoConfig, context: SinkTaskContext): MongoWriter = {
    ...
    settings.errorPolicy match {
      case RetryErrorPolicy() =>
        context.timeout(connectorConfig.getInt(MongoConfigConstants.ERROR_RETRY_INTERVAL_CONFIG).toLong)
      case _ =>
    }
...
object MongoClientProvider extends StrictLogging {
  ...
  private def getCredentials(settings: MongoSettings): MongoCredential = {
    ...
    authenticationMechanism match {
      ...
      case AuthenticationMechanism.MONGODB_CR =>
        MongoCredential.createMongoCRCredential(settings.username,
                                                settings.database,
                                                settings.password.value().toCharArray,
        )
      ...
      case AuthenticationMechanism.PLAIN =>
        MongoCredential.createPlainCredential(settings.username,
                                              settings.database,
                                              settings.password.value().toCharArray,
        )
      case AuthenticationMechanism.SCRAM_SHA_1 =>
        MongoCredential.createScramSha1Credential(settings.username,
                                                  settings.database,
                                                  settings.password.value().toCharArray,
        )
      case AuthenticationMechanism.SCRAM_SHA_256 =>
        MongoCredential.createScramSha256Credential(settings.username,
                                                    settings.database,
                                                    settings.password.value.toCharArray,
        )
    }
