...
import io.lenses.streamreactor.connect.gcp.storage.sink.config.SinkBucketOptions
import io.lenses.streamreactor.connect.gcp.storage.storage.GCPStorageFileMetadata

import java.io.File
import scala.collection.immutable

object GCPStorageWriterManagerCreator extends LazyLogging {
  ...
  def from(
    ...
    storageInterface: StorageInterface[GCPStorageFileMetadata],
  ): WriterManager[GCPStorageFileMetadata] = {
  ...
  private def bucketOptsForTopic(config: GCPStorageSinkConfig, topic: Topic): Option[SinkBucketOptions] =
  ...
  private def fatalErrorTopicNotConfigured(topicPartition: TopicPartition): SinkError =
    FatalCloudSinkError(
      s"Can't find the KCQL for source topic [${topicPartition.topic}]. The topics defined via [topics] or [topics.regex] need to have an equivalent KCQL statement: INSERT INTO {GCP_BUCKET} SELECT * FROM {TOPIC}.",
      topicPartition,
    )
}
