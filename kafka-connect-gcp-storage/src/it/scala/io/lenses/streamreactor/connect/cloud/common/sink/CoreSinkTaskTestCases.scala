...
abstract class CoreSinkTaskTestCases[
  ...
  unitUnderTest should "allow a primitive int key for _key partitioning" in {
    ...
    val props = (defaultProps + (
      s"$prefix.kcql" -> s"insert into $BucketName:$PrefixName select * from $TopicName PARTITIONBY _key STOREAS `CSV` PROPERTIES('padding.length.partition'='12', 'padding.length.offset'='12', '${FlushCount.entryName}'=1)",
    )).asJava
