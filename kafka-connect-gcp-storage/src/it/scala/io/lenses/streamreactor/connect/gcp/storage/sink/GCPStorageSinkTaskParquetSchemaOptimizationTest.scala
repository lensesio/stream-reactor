...
class GCPStorageSinkTaskParquetSchemaOptimizationTest
  ...
  "GCPStorageSinkTask" should "write to parquet format using latest schema optimization with interleaved schema versions" in {
    val props = (
      defaultProps ++
        Map(
          "connect.gcpstorage.kcql"                               -> s"insert into $BucketName:$PrefixName select * from $TopicName STOREAS PARQUET PROPERTIES('padding.length.partition'='12','padding.length.offset'='12','${FlushCount.entryName}'=6)",
          "connect.gcpstorage.latest.schema.optimization.enabled" -> "true",
        )
    ).asJava
    val expectedFile = "streamReactorBackups/schemaEvolutionTopic/000000000001/000000000006_10001_10006.parquet"

    val task = new GCPStorageSinkTask()
  ...
  "GCPStorageSinkTask" should "handle nested struct evolution with parquet and schema optimization" in {
    val props = (
      defaultProps ++
        Map(
          "connect.gcpstorage.kcql"                               -> s"insert into $BucketName:$PrefixName select * from $TopicName STOREAS PARQUET PROPERTIES('padding.length.partition'='12','padding.length.offset'='12','${FlushCount.entryName}'=4)",
          "connect.gcpstorage.latest.schema.optimization.enabled" -> "true",
        )
    ).asJava
