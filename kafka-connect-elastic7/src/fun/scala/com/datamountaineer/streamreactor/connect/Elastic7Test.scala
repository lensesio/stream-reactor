...
import io.lenses.streamreactor.connect.testcontainers.connect.BooleanCnfVal
...
class Elastic7Test extends AsyncFlatSpec with AsyncIOSpec with StreamReactorContainerPerSuite with Matchers {
  ...
  private def sinkConfig(): ConnectorConfiguration =
    ConnectorConfiguration(
      "elastic-sink",
      Map(
        "connector.class"              -> StringCnfVal("com.datamountaineer.streamreactor.connect.elastic7.ElasticSinkConnector"),
        ...
        "connect.progress.enabled"     -> BooleanCnfVal(true),
      ),
    )
