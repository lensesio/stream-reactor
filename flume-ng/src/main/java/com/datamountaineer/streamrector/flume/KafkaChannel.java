...
import static com.datamountaineer.streamrector.flume.KafkaChannelConfiguration.*;
...
public class KafkaChannel extends BasicChannelSemantics {
    ...
    public void configure(Context ctx) {
        ...
        kafkaConf.put(KEY_SERIALIZER_CLASS_KEY,
                io.confluent.kafka.serializers.KafkaAvroSerializer.class);

        kafkaConf.put(VALUE_SERIALIZER_CLASS_KEY,
                io.confluent.kafka.serializers.KafkaAvroSerializer.class);

        String schemaUrl = ctx.getString(SCHEMA_REGISTRY_KEY);
        if (schemaUrl == null || schemaUrl.isEmpty()) {
            throw new ConfigurationException(
                    "Schema Registry Url must be specified");
        }

        kafkaConf.put(SCHEMA_REGISTRY_KEY, schemaUrl);
    ...
    private class KafkaTransaction extends BasicTransactionSemantics {
        ...
        private Optional<HashMap<byte[], String>> serializedEvents = Optional.absent();
        ...
        protected void doPut(Event event) throws InterruptedException {
            ...
            if (!serializedEvents.isPresent()) {
                serializedEvents = Optional.of(new HashMap<byte[], String>());
            }
        ...
        protected void doCommit() throws InterruptedException {
            ...
            if (type.equals(TransactionType.PUT)) {
                try {
                    ...
                    for (byte[] event : serializedEvents.get().keySet()) {
                        messages.add(new KeyedMessage<String, byte[]>(topic.get(), serializedEvents.get().get(event), event));
                    }
