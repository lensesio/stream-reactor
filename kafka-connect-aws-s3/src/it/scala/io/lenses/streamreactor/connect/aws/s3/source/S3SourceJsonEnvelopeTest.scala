...
class S3SourceJsonEnvelopeTest extends S3ProxyContainerTest with AnyFlatSpecLike with Matchers with EitherValues {
  ...
  override def setUpTestData(): Unit = {
    ...
    try {
      ...
      bw.flush()
      bw.close()
      storageInterface.uploadFile(file, BucketName, s"$MyPrefix/json/0")
      ()
    } finally {
      file.delete()
      ()
