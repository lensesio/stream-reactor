...
class SinkDataExtractorTest extends AnyFlatSpec with Matchers {
  ...
  private val mapOfMapsSchema = SchemaBuilder.map(
    SchemaBuilder.string().build(),
    SchemaBuilder.map(SchemaBuilder.string().build(), SchemaBuilder.string().build())
      .build(),
  )
  ...
  "lookupFieldValueFromSinkData" should "convert boolean to string" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("mybool"))) should be(
      Right("true"),
    )
  }

  "lookupFieldValueFromSinkData" should "convert byte to string" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("mybytes"))) should be(
      Right("testBytes"),
    )
  }

  "lookupFieldValueFromSinkData" should "convert floats to string" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("myfloat32"))) should be(
      Right("32.0"),
    )
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("myfloat64"))) should be(
      Right("64.02"),
    )
  }

  "lookupFieldValueFromSinkData" should "convert ints to string" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("myint8"))) should be(
      Right("8"),
    )
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("myint16"))) should be(
      Right("16"),
    )
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("myint32"))) should be(
      Right("32"),
    )
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("myint64"))) should be(
      Right("64"),
    )
  }

  "lookupFieldValueFromSinkData" should "retain string as string" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("mystring"))) should be(
      Right("teststring"),
    )
  }

  "lookupFieldValueFromSinkData" should "return error when field not found" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("not-there"))) should be(
      Left(ExtractorError(MissingValue)),
    )
  }

  "lookupFieldValueFromSinkData" should "return error when non-primitive supplied" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("mystruct"))) should be(
      Left(ExtractorError(UnexpectedType)),
    )
  }

  "lookupFieldValueFromSinkData" should "handle nested string schema" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("mynestedschema",
                                                                                             "mynestedstring",
    ))) should be(Right("zip"))
  }

  "lookupFieldValueFromSinkData" should "handle complex map schema" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("mycomplexmap",
                                                                                             "mykey",
                                                                                             "mynestedstring",
    ))) should be(Right("wiz"))
  }
  ...
  "lookupFieldValueFromSinkData" should "handle struct containing map of maps" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(
      Some(PartitionNamePath("mymapofmaps", "c", "d")),
    ) should be(Right("2"))
  }

  "lookupFieldValueFromSinkData" should "handle array of structs" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("myarrayofstructs",
                                                                                             "0",
                                                                                             "mynestedstring",
    ))) should be(Right("wiz1"))
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("myarrayofstructs",
                                                                                             "1",
                                                                                             "mynestedstring",
    ))) should be(Right("wiz2"))
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath("myarrayofstructs",
                                                                                             "2",
                                                                                             "mynestedstring",
    ))) should be(Right("wiz3"))
  }

  "lookupFieldValueFromSinkData" should "handle structs containing array of primitives" in {
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath(
      "mystructswitharrayofprimitives",
      "myarray",
      "0",
    ))) should be(Right("sausages"))
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath(
      "mystructswitharrayofprimitives",
      "myarray",
      "1",
    ))) should be(Right("mash"))
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath(
      "mystructswitharrayofprimitives",
      "myarray",
      "2",
    ))) should be(Right("peas"))
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath(
      "mystructswitharrayofprimitives",
      "myarray",
      "3",
    ))) should be(Right("gravy"))
  }
  ...
  "lookupFieldValueFromSinkData" should "handle structs containing array of structs containing primitives" in {
    ...
    SinkDataExtractor.extractPathFromSinkData(StructSinkData(struct))(Some(PartitionNamePath(
      ...
      "3",
      "mynestedstring",
    ))) should be(Left(ExtractorError(MissingValue)))
  }
  ...
  "lookupFieldValueFromSinkData" should "handle 3d list sink data without schema" in {
    ...
    SinkDataExtractor.extractPathFromSinkData(arraySinkData)(Some(PartitionNamePath("1", "key3"))) should be(
      Right("val2"),
    )
  }
}
