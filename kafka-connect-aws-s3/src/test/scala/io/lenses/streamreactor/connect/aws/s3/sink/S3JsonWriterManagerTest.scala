...
class S3JsonWriterManagerTest extends AnyFlatSpec with Matchers with S3TestConfig {
  ...
  "json sink" should "write single json record" in {
    ...
    readFileToString(BucketName, "streamReactorBackups/myTopic/1/latest_1.json", blobStoreContext) should be("""{"name":"sam","title":"mr","salary":100.43}""")
  }
  ...
  "json sink" should "write schemas to json" in {
    ...
    readFileToString(BucketName, "streamReactorBackups/myTopic/1/latest_3.json", blobStoreContext) should be("""{"name":"sam","title":"mr","salary":100.43}{"name":"laura","title":"ms","salary":429.06}{"name":"tom","title":null,"salary":395.44}""")
  }
