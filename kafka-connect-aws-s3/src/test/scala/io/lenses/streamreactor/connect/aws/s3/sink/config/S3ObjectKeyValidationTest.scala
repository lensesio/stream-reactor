...
class S3ObjectKeyValidationTest extends AnyFunSuite with Matchers {
  ...
  test("bucket with invalid characters should be invalid") {
    val bucket = S3ObjectKey("buck et", "prefix".some)
    S3ObjectKey.validate(bucket).isValid shouldBe false
  }
  test("bucket with invalid prefix should be invalid") {
    val bucket = S3ObjectKey("bucket", "pre fix".some)
    S3ObjectKey.validate(bucket).isValid shouldBe false
  }
  ...
  test("bucket starting with sthree- is invalid") {
    val bucket = S3ObjectKey("sthree-bucket", "aws".some)
    S3ObjectKey.validate(bucket).isValid shouldBe false
  }
  test("bucket starting with xn-- is invalid") {
    val bucket = S3ObjectKey("xn--bucket", "aws".some)
    S3ObjectKey.validate(bucket).isValid shouldBe false
  }
  ...
  test("valid prefix is valid") {
    S3ObjectKey.validate(S3ObjectKey("bucket", "ab.c/x/y/z".some)).isValid shouldBe true
    S3ObjectKey.validate(S3ObjectKey("bucket", "4a/b/c".some)).isValid shouldBe true
  }
  test("prefix starting with / or ending with / is invalid") {
    S3ObjectKey.validate(S3ObjectKey("bucket", "/ab/c".some)).isValid shouldBe false
    S3ObjectKey.validate(S3ObjectKey("bucket", "ab/c/".some)).isValid shouldBe false
  }
