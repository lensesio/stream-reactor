...
class S3ParquetWriterManagerTest extends AnyFlatSpec with Matchers with S3TestConfig {
  ...
  private val parquetConfig = S3SinkConfig(S3Config(Some(Identity), Some(Credential), AuthMode.Credentials),
    Gen1,
    Set(
      SinkBucketOptions(
        TopicName,
  ...
  "parquet sink" should "write 2 records to parquet format in s3" in {
    ...
    genericRecords.head.get("name").toString should be("sam")
  ...
  "parquet sink" should "write start a new file in case of schema change" in {
    ...
    genericRecords1.head.get("name").toString should be("sam")
    ...
    genericRecords2.head.get("name").toString should be("tom")
    ...
    genericRecords3.head.get("name").toString should be("bobo")
