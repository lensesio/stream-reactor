...
/**
  * Builds the Hbase row key for the given connect record and payload. The implementations would decide what is using and how.
  */
trait RowKeyBuilder {
...
/**
  ...
  */
class GenericRowKeyBuilder(keyDelimiter: String = "\n") extends RowKeyBuilder {
  ...
  override def build(record: SinkRecord, payload: Any): Array[Byte] = {
    Bytes.add(
      Array(
        ...
        delimiterBytes,
        Bytes.toBytes(record.kafkaPartition()),
        delimiterBytes,
        Bytes.toBytes(record.kafkaOffset())
      )
    )
  }
}
...
/**
  * Creates an Hbase row key based on the connect SinkRecord instance key. Only connect Schema primitive types are handled
  */
class SinkRecordKeyRowKeyBuilder extends RowKeyBuilder {
  override def build(record: SinkRecord, payload: Any): Array[Byte] = {
...
/**
  ...
  */
class AvroRecordRowKeyBuilder(valuesExtractorsMap: Map[String, (Any) => Array[Byte]],
                              keys: Seq[String],
                              keyDelimiter: String = "\n") extends RowKeyBuilder {
...
case class StructFieldsRowKeyBuilder(keys: Seq[String],
                                     keyDelimiter: String = "\n") extends RowKeyBuilder {
  require(keys.nonEmpty, "Invalid keys provided")
  ...
  override def build(record: SinkRecord, payload: Any): Array[Byte] = {
    ...
    val keyBytes = keys.map { case key =>
