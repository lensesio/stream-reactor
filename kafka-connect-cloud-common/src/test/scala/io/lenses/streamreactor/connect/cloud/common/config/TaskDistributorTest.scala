...
class TaskDistributorTest extends AnyWordSpec with Matchers with TaskIndexKey {
  "TaskDistributor" should {
    "distribute tasks" in {
      val tasks =
        new TaskDistributor(connectorPrefix).distributeTasks(Map.empty[String, String].asJava, 5).asScala.map(
          _.asScala.toMap,
        ).toList
      tasks shouldBe List(
        Map(TASK_INDEX -> "0:5"),
    ...
    "handle 0 max tasks" in {
      val tasks =
        new TaskDistributor(connectorPrefix).distributeTasks(Map.empty[String, String].asJava, 0).asScala.map(
          _.asScala.toMap,
        ).toList
      tasks shouldBe List()
    }
