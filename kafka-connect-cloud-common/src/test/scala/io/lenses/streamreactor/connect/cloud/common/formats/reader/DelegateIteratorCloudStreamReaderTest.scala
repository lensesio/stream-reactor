...
import io.lenses.streamreactor.connect.cloud.common.formats.reader.DelegateIteratorCloudStreamReader
...
class DelegateIteratorCloudStreamReaderTest extends AnyFunSuite with Matchers {
  test("returns 0 if the inner iterator is empty") {
    val innerIterator = new CloudDataIterator[Int] {
      override def hasNext: Boolean = false
      override def next():  Int     = throw new NoSuchElementException

      override def close(): Unit = {}
    }
    val delegateIterator = new DelegateIteratorCloudStreamReader[Int](innerIterator, null, null)
  ...
  test("returns 0 if the inner iterator is not empty") {
    ...
    val delegateIterator = new DelegateIteratorCloudStreamReader[Int](innerIterator, converter, null)
  ...
  test("returns all the inner iterator elements") {
    ...
    val inner = new CloudDataIterator[Int] {
    ...
    val delegateIterator = new DelegateIteratorCloudStreamReader[Int](inner, converter, null)
    delegateIterator.hasNext shouldBe true
    delegateIterator.currentRecordIndex shouldBe -1
