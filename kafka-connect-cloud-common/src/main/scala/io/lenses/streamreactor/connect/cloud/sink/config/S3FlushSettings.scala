 ...
 */
package io.lenses.streamreactor.connect.cloud.sink.config
...
import io.lenses.streamreactor.connect.cloud.sink.commit.CommitPolicyCondition
...
trait S3FlushConfigKeys extends WithConnectorPrefix {
  val DISABLE_FLUSH_COUNT: String = s"$connectorPrefix.disable.flush.count"

}

trait S3FlushSettings extends BaseSettings with S3FlushConfigKeys {
