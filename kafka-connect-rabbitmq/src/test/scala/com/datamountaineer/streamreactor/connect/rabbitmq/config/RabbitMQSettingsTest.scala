...
class RabbitMQSettingsTest extends WordSpec with Matchers with TestBase {
    "RabbitMQSettings" should {
        "parse props correctly when creating a new instance with a single KCQL" in {
            ...
            settings.kcql.toList(0).getSource shouldBe QUEUES(0)
            settings.kcql.toList(0).getTarget shouldBe KAFKA_TOPICS(0)
        }

        "parse props correctly when creating a new instance with multiple KCQLs" in {
            val props = getProps4KCQLNoConverters()
            val settings = RabbitMQSettings(props)
            ...
            settings.kcql.map(_.getSource).toList should contain theSameElementsInOrderAs QUEUES
            settings.kcql.map(_.getTarget).toList should contain theSameElementsInOrderAs KAFKA_TOPICS
        }

        "initialize the correct converter with respect to WITHCONVERTER attibute in KCQL" in {
            val props = getProps4KCQLsWithAllConverters()
            val settings = RabbitMQSettings(props)

            settings.sourcesToConvertersMap.getOrElse(QUEUES(0),fail) shouldBe a [BytesConverter]
            settings.sourcesToConvertersMap.getOrElse(QUEUES(1),fail) shouldBe a [JsonSimpleConverter]
            settings.sourcesToConvertersMap.getOrElse(QUEUES(2),fail) shouldBe a [JsonConverterWithSchemaEvolution]
            settings.sourcesToConvertersMap.getOrElse(QUEUES(3),fail) shouldBe an [AvroConverter]
        }

        "throw a ConfigException if HOST is not provided" in {
            val props = getProps1KCQLNoConvertersNoHost()
