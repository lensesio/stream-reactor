  ...
  public Field(String name, String alias, FieldType fieldType, List<String> parents) {
    if (name == null || name.trim().length() == 0) {
      throw new IllegalArgumentException(
          String.format("field is not valid:<%s>", String.valueOf(name)));
    }
    if (alias == null || alias.trim().length() == 0) {
      throw new IllegalArgumentException(
          String.format("alias is not valid:<%s>", String.valueOf(alias)));
    }
    this.name = name;
    this.alias = alias;
        ...
        switch (name.toLowerCase()) {
          ...
          default:
            if (parents.size() <= 1 || !"key".equals(parents.get(1).toLowerCase())) {
              throw new IllegalArgumentException(
                  String.format(
                      "Invalid syntax. '_' needs to be followed by: key,%s,%s,%s,%s",
