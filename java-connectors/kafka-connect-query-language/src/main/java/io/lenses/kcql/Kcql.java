...
public class Kcql {
  ...
  private void addField(final Field field) {
    ...
    if (fieldExists(field)) {
      throw new IllegalArgumentException(
          String.format("Field %s has already been defined", field.getName()));
    }
    fields.add(field);
  }
  ...
  private void addKeyField(final Field field) {
    ...
    if (fieldExists(field)) {
      throw new IllegalArgumentException(
          String.format("Key field %s has already been defined", field.getName()));
    }
    keyFields.add(field);
  }
  ...
  private void addHeaderField(final Field field) {
    ...
    if (fieldExists(field)) {
      throw new IllegalArgumentException(
          String.format("Header field %s has already been defined", field.getName()));
    }
  ...
  public static Kcql parse(final String syntax) {
    ...
    parser.addParseListener(
        new ConnectorParserBaseListener() {

          @Override
          public void exitWith_subscription_value(
              ConnectorParser.With_subscription_valueContext ctx) {
          ...
          public void exitLimit_value(ConnectorParser.Limit_valueContext ctx) {
            try {
              ...
              if (limit < 1)
                throw new IllegalArgumentException(
                    "Invalid limit specified. Needs to be an integer greater than zero");
              kcql.limit = limit;
          ...
          public void exitStoreas_parameter(ConnectorParser.Storeas_parameterContext ctx) {
            ...
            for (String key : kcql.getStoredAsParameters().keySet()) {
              if (key.compareToIgnoreCase(value) == 0) {
                throw new IllegalArgumentException(
                    value + " is a duplicated entry in the storeAs parameters list");
              }
            }
          ...
          public void exitBatch_size(ConnectorParser.Batch_sizeContext ctx) {
            ...
            try {
              ...
              if (newBatchSize <= 0) {
                throw new IllegalArgumentException(
                    value + " is not a valid number for a batch Size.");
              }
          ...
          public void exitTimestamp_unit_value(ConnectorParser.Timestamp_unit_valueContext ctx) {
            ...
            try {
              ...
              throw new IllegalArgumentException(
                  ("Invalid 'TIMESTAMPUNIT'. Available values are : " + sb));
            }
          }
          ...
          public void exitWith_format(ConnectorParser.With_formatContext ctx) {
            try {
              ...
              throw new IllegalArgumentException(
                  ("Invalid 'FORMAT'. Available values are : " + sb));
            }
          }
          ...
          public void exitTag_key(ConnectorParser.Tag_keyContext ctx) {
            if (ctx.getText().trim().endsWith(".")) {
              throw new IllegalArgumentException(
                  "Invalid syntax for tags. Field selection can not end with '.'");
            }
          ...
          public void exitTag_definition(ConnectorParser.Tag_definitionContext ctx) {
            ...
            if (tagValue[0] != null) {
              ...
              if (tmp.startsWith("=")) {
                ...
                throw new IllegalArgumentException(
                    "Invalid syntax for tags. Needs to be 'tag1 [as x]' or 'tag1' or 'tag1 ="
                        + " constant'");
              }
            }
          ...
          public void exitKey_delimiter_value(ConnectorParser.Key_delimiter_valueContext ctx) {
            ...
            if (kcql.keyDelimiter.trim().length() == 0) {
              throw new IllegalArgumentException(
                  "Invalid key delimiter. Needs to be a non empty string.");
            }
          }
