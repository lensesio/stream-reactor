...
class MqttManager(
  connectionFn:  MqttSourceSettings => MqttConnectOptions,
  convertersMap: Map[String, Converter],
  settings:      MqttSourceSettings,
) extends AutoCloseable
    with StrictLogging
    with MqttCallbackExtended {
  ...
  override def messageArrived(topic: String, message: MqttMessage): Unit = {
    if (settings.logMessageReceived) {
      ...
      logger.debug(
        s"Message received on topic [$topic]. Message id =[${message.getId}] , isDuplicate=${message.isDuplicate}, payload=$msg",
      )
    }
    ...
    val kcql = sourceToTopicMap
      .getOrElse(
        ...
        throw new ConfigException(
          s"Topic $topic is not configured. Available topics are:${sourceToTopicMap.keySet.mkString(",")}",
        ),
      )
    ...
    if (!message.isDuplicate || (settings.processDuplicates && message.isDuplicate)) {
      try {
        ...
        Option(converter.convert(kafkaTopic,
                                 sourceTopic,
                                 message.getId.toString,
                                 message.getPayload,
                                 keys,
                                 kcql.getKeyDelimeter,
        )) match {
          case Some(record) =>
            queue.add(record)
          ...
          case None =>
            logger.warn(
              s"Error converting message with id:${message.getId} on topic:$topic. 'null' record returned by converter",
            )
