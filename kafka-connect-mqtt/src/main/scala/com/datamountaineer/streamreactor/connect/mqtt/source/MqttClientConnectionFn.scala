...
object MqttClientConnectionFn extends StrictLogging {
  def apply(callback: MqttCallback)(implicit settings: MqttSourceSettings) = {
    {
      ...
      settings.sslCertFile.foreach { _ =>
        options.setSocketFactory(
          SSLSocketFactory(settings.sslCACertFile.get, settings.sslCertFile.get, settings.sslCertKeyFile.get, "")
        )
      }
