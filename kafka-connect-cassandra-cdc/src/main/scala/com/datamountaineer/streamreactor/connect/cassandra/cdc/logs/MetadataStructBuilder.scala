...
object MetadataStructBuilder {
  def apply(pu: PartitionUpdate)(implicit dataProvider: SubscriptionDataProvider, config: CdcConfig): Struct = {
    val struct = new Struct(ConnectSchemaBuilder.MetadataSchema)

    struct.put(ConnectSchemaBuilder.TimestampField, pu.maxTimestamp())
    struct
  }
}
