...
class PulsarSourceConnectorTest extends WordSpec with Matchers {
  ...
  "should start a connector with shared consumer" in {
    val kcql = s"INSERT INTO kafka_topic SELECT * FROM $pulsarTopic BATCH 10 WITHTYPE SHARED"
    val kcql1 = s"INSERT INTO kafka_topic SELECT * FROM $pulsarTopic1 BATCH 10 WITHTYPE FAILOVER"
  ...
  "should fail to start a connector with exclusive consumer and more than one task" in {
    val props =  Map(
      PulsarConfigConstants.HOSTS_CONFIG -> "pulsar://localhost:6650",
      PulsarConfigConstants.KCQL_CONFIG -> s"INSERT INTO kafka_topic SELECT * FROM $pulsarTopic BATCH 10 WITHTYPE exclusive;INSERT INTO kafka_topic SELECT * FROM $pulsarTopic BATCH 10 WITHTYPE FAILOVER",
      PulsarConfigConstants.THROW_ON_CONVERT_ERRORS_CONFIG -> "true",
      PulsarConfigConstants.POLLING_TIMEOUT_CONFIG -> "500"
    ).asJava
