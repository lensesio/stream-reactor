...
object PulsarConfigConstants {
  ...
  val KCQL_DOC =
    "Contains the Kafka Connect Query Language describing the flow from Apache Pulsar to Apache Kafka topics"
  ...
  val POLLING_TIMEOUT_DOC =
    s"Provides the timeout to poll incoming messages in milliseconds. Connect will write to Kafka is this reached of the $CONNECTOR_PREFIX.$BATCH_SIZE_PROP_SUFFIX. Which ever is first."
  ...
  val INTERNAL_BATCH_SIZE_DOC =
    s"The number of records Connect will wait before writing to Kafka. The connector will return if $CONNECTOR_PREFIX.polling.timeout is reached first."
