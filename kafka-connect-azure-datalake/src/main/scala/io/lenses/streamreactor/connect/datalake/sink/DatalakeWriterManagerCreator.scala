...
import io.lenses.streamreactor.connect.datalake.sink.config.SinkBucketOptions
...
object DatalakeWriterManagerCreator extends LazyLogging {
  ...
  def from(
    ...
    storageInterface: StorageInterface[DatalakeFileMetadata],
  ...
  private def bucketOptsForTopic(config: DatalakeSinkConfig, topic: Topic): Option[SinkBucketOptions] =
